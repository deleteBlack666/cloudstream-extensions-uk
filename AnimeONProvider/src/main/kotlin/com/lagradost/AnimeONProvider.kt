...
class AnimeONProvider : MainAPI() {
            ...
            for (player in item.player) {
                val realVideoUrl = if (episodeId != null) {
                    try {
                        ...
                        if (epDetailJson != null) Gson().fromJson(epDetailJson, FundubEpisode::class.java).videoUrl else null
                    } catch (e: Exception) { null }
                } else null
                ...
                if (!fileUrl.isNullOrEmpty()) {
                    M3u8Helper.generateM3u8("${item.translation.name} (${player.name})", fileUrl, "https://ashdi.vip").dropLast(1).forEach(callback)
                    break
                }
                ...
                if (!videoUrl.isNullOrEmpty()) {
                    if (videoUrl.contains("moonanime.art")) {
                        if (videoUrl.contains("m3u8")) {
                            M3u8Helper.generateM3u8("${item.translation.name} (${player.name})", videoUrl, "https://moonanime.art/").dropLast(1).forEach(callback)
                            break
                        }
                        ...
                        if (rawFile.isNotEmpty()) {
                            ...
                            if (rawFile.startsWith("[")) {
                                ...
                                qualityRegex.findAll(rawFile).forEach { match ->
                                    ...
                                    M3u8Helper.generateM3u8("$sourceName $quality", url, "https://moonanime.art/", 
                                        headers = mapOf("User-Agent" to userAgent, "Referer" to "https://animeon.club/")).dropLast(1).forEach(callback)
                                }
                            } else if (rawFile.contains(".m3u8")) {
                                M3u8Helper.generateM3u8(sourceName, rawFile, "https://moonanime.art/", 
                                    headers = mapOf("User-Agent" to userAgent, "Referer" to "https://animeon.club/")).dropLast(1).forEach(callback)
                            }
                            break
                        }
    ...
    private suspend fun processAshdiIframe(iframeUrl: String, sourceName: String, callback: (ExtractorLink) -> Unit) {
        try {
            ...
            fileRegex.find(html)?.groupValues?.get(1)?.let { streamUrl ->
                M3u8Helper.generateM3u8(sourceName, streamUrl, "https://ashdi.vip/").dropLast(1).forEach(callback)
            }
        } catch (e: Exception) { }
    }
    ...
    private suspend fun getMoonFile(iframeUrl: String): String {
        val html = app.get(iframeUrl, headers = mapOf("User-Agent" to userAgent, "Referer" to "https://animeon.club/")).text
    ...
    private fun extractIntFromString(string: String): Int? {
        val match = Regex("(\\d+)").find(string) ?: return null
        val value = match.value
        return if (value.startsWith("0") && value.length > 1) value.drop(1).toIntOrNull() else value.toIntOrNull()
    }
}
