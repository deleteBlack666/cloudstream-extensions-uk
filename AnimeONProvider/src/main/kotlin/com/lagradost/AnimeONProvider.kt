...
class AnimeONProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        if (!request.data.contains("pageIndex") && page != 1) return newHomePageResponse(request.name, emptyList())
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val animeJSON = Gson().fromJson(
            app.get(
                searchApi.format(query),
                headers = mapOf("Referer" to "https://animeon.club/anime?search=")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        Gson().fromJson(
            app.get(
                "$mainUrl/api/player/episodes/${animeJSON.id}/${fundubs?.get(0)?.player?.get(0)?.id}/${fundubs?.get(0)?.fundub?.id}",
                headers = mapOf(
                    "Referer" to "https://animeon.club/",
                    "User-Agent" to "Mozilla/5.0 (Windows NT 10.0; rv:126.0) Gecko/20100101 Firefox/126.0",
                )
    ...
    override suspend fun loadLinks(
        ...
        if (dataList.size == 2) {
            fundubs.map { dub ->
                Gson().fromJson(
                    app.get(
                        "$mainUrl/api/player/episodes/${dataList[0]}/${dub.player[0].id}/${dub.fundub.id}",
                        headers = mapOf("Referer" to "https://animeon.club/")
                    ).text, PlayerEpisodes::class.java
