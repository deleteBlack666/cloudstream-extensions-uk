...
class AnimeONProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // Спроба отримати епізоди через HTML (API не працює через Cloudflare)
        try {
            val doc = app.get(url).document
            // Шукаємо посилання, які схожі на епізоди
            ...
            var foundLinks = mutableListOf<org.jsoup.nodes.Element>()
            for (selector in selectors) {
                val elements = doc.select(selector)
                if (elements.isNotEmpty()) {
                    foundLinks = elements.toList().toMutableList()
                    break
                }
            }
            // Якщо нічого не знайшли, беремо всі посилання, що ведуть на /anime/число, окрім поточного
            if (foundLinks.isEmpty()) {
                val allLinks = doc.select("a[href^='/anime/']")
                foundLinks = allLinks.toList().filter { it.attr("href") != "/anime/$animeId" }.toMutableList()
            }
            ...
            // Ігноруємо помилки парсингу HTML
        }
